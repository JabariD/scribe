...
- Settings stored in `~/Library/Application Support/scribe/config.json` (`model`, `show_recording_overlay`, `realtime_transcription_enabled`, `prompt`, `post_process_enabled`, `post_process_prompt`; legacy `api_key` is migrated to Keychain on read)
